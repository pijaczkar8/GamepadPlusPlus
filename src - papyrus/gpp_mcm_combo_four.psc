Scriptname gpp_mcm_combo_four extends gpp_mcm_page
...
; ### SETUP ###
event OnInit()
	if RegisterModule("$gpp_mcm_lbl_combo_four", 5) != OK
	...
	if KH.bFourthComboEnabled
		...
		if keyCode > 0
		    ...
		    if !KH.biEquipLoaded || KH.aiiEquipKeys.Find(keyCode) == -1
				...
				MCM.AddTextOptionST("C4_txt_iEquipConflict_1", "$gpp_mcm_com_txt_iEquipConflict{" + keyName + "}", "")
			...
			MCM.AddTextOptionST("C4_txt_C4NoKey", "$gpp_mcm_com_txt_noKey1", "")
			MCM.AddTextOptionST("C4_txt_C4NoKey2", "$gpp_mcm_com_txt_noKey2", "")
		endIf
...
int function SaveData()             ; sC4Ave page data and return jObject
